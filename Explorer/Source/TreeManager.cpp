...
file_header get_file_header(std::fstream *src_file);
...
{
  // create file
  std::fstream dest_file(filename, std::fstream::out | std::fstream::binary);
...
bool delete_node(node *cur_node)
{
  ...
  // open source file
  std::fstream src_file(src_drive_node->filename, std::fstream::in | std::fstream::binary);
  ...
  // copy header bytes 'RNBWFS' 0x1a 0x?? (version)
...
bool rename_node(node *cur_node, std::string new_path)
{
  ...
  else
  {
    ...
    dest_file.open(dest_drive_node->filename, std::fstream::in | std::fstream::out | std::fstream::binary);

    // error check
    if (dest_file.fail())
      throw TREE_MANAGER_ERRORS::FAILED_TO_OPEN_DESTINATION_FILE;

    // set file cursor to last byte of the file
    dest_file.seekg(0, std::fstream::end);
    int size = dest_file.tellg();
    size = size - 1;
...
{
  // open source file
  std::fstream src_file(filename.c_str(), std::fstream::in | std::fstream::binary);
...
bool add_file(node *parent, std::string filename)
{
  ...
  // open destination file
  std::fstream dest_file(drive_node->filename, std::fstream::in | std::fstream::out | std::fstream::ate | std::fstream::binary);
  ...
  std::fstream src_file(filename, std::fstream::in | std::fstream::ate | std::fstream::binary);
...
bool get_file_content(node *file_node, uint8_t **data, int *data_size)
{
  ...
  // try to open file
  std::fstream src_file(drive_node->filename, std::fstream::in | std::fstream::binary);
